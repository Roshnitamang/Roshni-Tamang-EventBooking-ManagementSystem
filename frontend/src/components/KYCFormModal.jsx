        ...
        <div className="flex justify-between items-center mb-10 px-4">
            {[1, 2, 3, 4].map((i) => (
                <div key={i} className="flex flex-col items-center gap-2 flex-1">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black transition-all ${step >= i ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30 scale-110' : 'bg-gray-100 dark:bg-gray-800 text-gray-400'}`}>
                        {step > i ? <Check className="w-5 h-5" /> : i}
                    </div>
                    <span className={`text-[8px] font-black uppercase tracking-widest ${step >= i ? 'text-blue-600' : 'text-gray-400'}`}>
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
            <motion.div
                ...
                className="bg-white dark:bg-gray-900 w-full max-w-4xl rounded-[3rem] overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-800"
            >
                {/* Header */}
                <div className="p-8 border-b border-gray-50 dark:border-gray-800 flex justify-between items-center bg-gradient-to-r from-blue-600 to-indigo-800 text-white shadow-xl">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-blue-100 text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mt-1">eSewa Standard Compliance</p>
                        </div>
                    </div>
                ...
                <div className="p-10">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8 max-h-[60vh] overflow-y-auto custom-scrollbar pr-4">
                        <AnimatePresence mode="wait">
                            {step === 1 && (
                                <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                                    <h3 className="text-xl font-black text-gray-900 dark:text-white mb-6">Personal details</h3>
                            ...
                            {step === 3 && (
                                <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                                    <h3 className="text-xl font-black text-gray-900 dark:text-white mb-6">Identity details</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {step === 4 && (
                                <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-xl font-black text-gray-900 dark:text-white">Document Uploads</h3>
                                        <span className="px-3 py-1 bg-blue-50 text-blue-600 text-[10px] font-black uppercase rounded-full">Final Step</span>
                                    </div>

                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                        <div className="space-y-4 text-center lg:text-left">
                                            <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-1">1. Profile Selfie</p>
                                            <div className="flex flex-col items-center gap-4 bg-gray-50 dark:bg-gray-800/50 p-6 rounded-[2.5rem] border border-dashed border-gray-200 dark:border-gray-700">
                                                <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-white dark:border-gray-800 shadow-xl relative group bg-gray-200 dark:bg-gray-700">
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-tighter">Clear Background</p>
                                                    <p className="text-[10px] text-gray-400 italic">No glasses or masks</p>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="lg:col-span-2 space-y-4">
                                            <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-1">2. Identity Documents (Front & Back)</p>
                                            ...
                                            <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30 flex gap-3">
                                                ...
                                                <p className="text-[9px] text-blue-800 dark:text-blue-200 font-black uppercase leading-relaxed tracking-wider">
                        ...
                        {/* Footer Actions */}
                        <div className="flex gap-4 pt-6 border-t border-gray-50 dark:border-gray-800 sticky bottom-0 bg-white dark:bg-gray-900 py-4">
                            {step > 1 && (
                                <button type="button" onClick={() => setStep(step - 1)} className="flex-1 py-5 rounded-2xl font-black uppercase tracking-widest bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 transition-all active:scale-95">Back</button>
                            )}
                            <button
                                type="submit"
                                disabled={loading}
                                className={`flex-[2] py-5 rounded-3xl font-black uppercase tracking-[0.2em] text-white shadow-2xl transition-all ${loading ? 'opacity-50' : 'bg-gradient-to-r from-blue-600 to-indigo-700 hover:shadow-blue-500/20 active:scale-[0.98]'}`}
                            >
...
const InputField = ({ label, ...props }) => (
    <div className="space-y-2 group">
        <label className="text-[10px] font-black uppercase text-gray-400 group-focus-within:text-blue-600 tracking-widest ml-1 transition-colors">{label}</label>
        <input
            required
            className="w-full bg-gray-50 dark:bg-gray-800 border border-transparent rounded-2xl p-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none dark:text-white font-bold transition-all"
            {...props}
        />
    </div>
);
...
const SelectField = ({ label, options, ...props }) => (
    <div className="space-y-2 group">
        ...
        <select
            required
            className="w-full bg-gray-50 dark:bg-gray-800 border border-transparent rounded-2xl p-4 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none dark:text-white font-bold transition-all appearance-none cursor-pointer"
            {...props}
        >
...
const FileUpload = ({ label, name, preview, onChange }) => (
    <div className="space-y-2">
        ...
        <div className="relative group overflow-hidden bg-white dark:bg-gray-900 rounded-2xl aspect-video border-2 border-dashed border-gray-100 dark:border-gray-800 hover:border-blue-500/50 transition-all flex items-center justify-center">
            {preview ? (
                <img src={preview} className="w-full h-full object-cover" alt={label} />
            ) : (
                <div className="text-center">
                    <Upload className="w-6 h-6 text-gray-300 mx-auto mb-2 group-hover:text-blue-500 transition-colors" />
                    <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Tap to Upload</p>
                </div>
            )}
            <input
                type="file"

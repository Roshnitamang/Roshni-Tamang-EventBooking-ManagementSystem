        ...
        <nav className="fixed top-0 left-0 right-0 z-50 glass-effect">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex items-center h-20 gap-8">
                    ...
                    {/* PRIMARY NAV LINKS */}
                    {userData ? (
                        <ul className={`hidden lg:flex items-center gap-10 text-[11px] font-bold uppercase tracking-[0.2em] text-zinc-500 ${userData.role !== 'user' ? 'flex-1 justify-center' : ''}`}>
                            <li>
                            ...
                            {userData.role === 'user' && (
                                <li>
                                    <NavLink to="/my-bookings" className={({ isActive }) => `hover:text-emerald-400 transition-colors ${isActive ? 'text-emerald-500' : ''}`}>
                                        Bookings
                                    </NavLink>
                                </li>
                            )}
                            ...
                            {userData.role === 'organizer' && (
                                <li>
                                    <button
                                        onClick={() => navigate('/organizer-dashboard', { state: { activeStep: 'build' } })}
                                        className="hover:text-emerald-400 transition-all flex items-center gap-2 group/btn"
                                    >
                                        <Plus className="w-3 h-3 text-emerald-500 group-hover/btn:rotate-90 transition-transform" />
                                        Create Event
                                    </button>
                                </li>
                            )}
                            ...
                            {(userData.role === 'admin' || userData.role === 'super-admin') && (
                                <li>
                                    <button
                                        onClick={() => navigate(dashboardPath, { state: { activeStep: 'organizers' } })}
                            ...
                            <li>
                                <NavLink to="/help" className={({ isActive }) => `hover:text-emerald-400 transition-colors ${isActive ? 'text-emerald-500' : ''}`}>
                                    Help
                                </NavLink>
                            </li>
                        </ul>
